...
<body>
    ...
    <div class="container" style="padding: 20px;">
        ...
        <div class="video-grid">
            <%-- Giả định có một danh sách video (videoList) từ Servlet được đặt trong request scope --%>
            <c:forEach var="video" items="${videoList}" varStatus="loop">
                <div class="video-item">
                    <a href="/detail?id=${video.id}">
                        <div class="poster-box">
                            <img src="${video.posterUrl}" alt="${video.title}" class="poster-image"/>
                            <div class="poster-text">POSTER</div>
                        </div>
                    </a>
                    <p class="video-title">${video.title}</p>
                    <div class="action-buttons">
                        <%-- Logic [Like].Click: Ghi nhận yêu thích của người dùng. Cần Login. --%>
                        <a href="/like?id=${video.id}" class="btn btn-like">Like</a>
        ...
        <div class="pagination-controls">
            <%-- Giả định các biến page, totalPages, maxItemsPerPage được đặt trong request scope --%>
            <c:set var="currentPage" value="${param.page != null ? param.page : 1}" />
            
            <%-- [|<] Trang đầu --%>
            <a href="/home?page=1" class="page-link ${currentPage == 1 ? 'disabled' : ''}">|&lt;</a>
            
            <%-- [<<] Trang trước --%>
            <c:set var="prevPage" value="${currentPage - 1 > 0 ? currentPage - 1 : 1}" />
            <a href="/home?page=${prevPage}" class="page-link ${currentPage == 1 ? 'disabled' : ''}">&lt;&lt;</a>

            <%-- [>>] Trang sau --%>
            <c:set var="nextPage" value="${currentPage + 1 <= totalPages ? currentPage + 1 : totalPages}" />
            <a href="/home?page=${nextPage}" class="page-link ${currentPage == totalPages ? 'disabled' : ''}">&gt;&gt;</a>

            <%-- [>|] Trang cuối --%>
            <a href="/home?page=${totalPages}" class="page-link ${currentPage == totalPages ? 'disabled' : ''}">&gt;|</a>
        </div>
    </div>
